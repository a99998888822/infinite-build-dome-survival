...
1. `icon` 是角色唯一 UI 图像路径，角色选择页面、营地角色信息、UI 列表和存档展示统一复用。
2. `start_weapons` 保存初始武器 ID，并引用 `weapons.json`。
3. 角色场景表现只使用右向基础单帧和右向行走帧表；向左移动时由 Godot 水平翻转。
4. 当前不配置独立角色选择头像，也不配置向上、向下动画资源。
...
9. `hit_radius` 只表示武器基础命中/影响半径；近战、远程、范围武器的最终攻击范围统一由 `area_size` 加成。
10. `area_size` 是唯一攻击范围加成属性；`pickup_radius` 只控制掉落物吸附，不参与武器攻击范围。
