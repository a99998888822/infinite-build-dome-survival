...
6. 索敌规则不写入配置，局内武器模块默认按最近敌人索敌。
7. 投射物/攻击模式暂不配置化，等武器差异变复杂后再单独设计。
8. `hit_radius` 只表示武器基础命中/影响半径；近战、远程、范围武器的最终攻击范围统一由 `area_size` 加成。
9. `area_size` 是唯一攻击范围加成属性；`pickup_radius` 只控制掉落物吸附，不参与武器攻击范围。
