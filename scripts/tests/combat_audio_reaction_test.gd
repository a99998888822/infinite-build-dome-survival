...
func _run() -> void:
	...
	hit(enemies[0])
	await drain()
	check(not heard.has("ice") and not heard.has("reaction_freeze") and enemies[0].has_status("slowed"), "ordinary ice applies slow without audio")
	check(is_equal_approx(float(gains.get("wood_arrow", 0)), -7.0), "ordinary weapon hit retains its original gain")

	await prepare(["scroll_water"], [Vector2.ZERO, Vector2(250, 0)])
	enemies[0].apply_burning(3.0, 10.0, "audio_test")
	hit(enemies[0])
	hit(enemies[1])
	await drain()
	check(heard.has("reaction_steam") and heard.has("water"), "different same-frame contacts never suppress each other's base effect")
