...
| 局内战斗循环 | 单局状态、时间轴、经验、升级、胜负结算 | 18分钟闯关闭环 | 无尽模式、难度曲线、多事件时间轴 |
...
| 敌人与波次 | 怪物属性、生成器、AI追踪、精英/BOSS刷新 | 普通怪 + 1个精英/BOSS | 多敌人类型、词缀、阶段Boss |
...
| 掉落与奖励 | 经验球、金币/星点、遗物/武器选项池 | 经验球 + 升级三选一 | 稀有度、权重、裂隙奖励、结算奖励 |
| 局外营地与成长 | 营地建筑、升级选项、解锁、存档 | 固定营地界面 + 3个建筑 | 5建筑、图鉴、纪录、解锁路线 |
| UI交互 | HUD、升级弹窗、负载替换、羁绊面板、营地界面 | 战斗HUD + 升级弹窗 | 完整菜单、营地升级树、结算、图鉴 |
| 音频与氛围表现 | BGM/SFX管理、克苏鲁氛围反馈、粒子/屏幕后处理 | 基础攻击/拾取/UI音效 | 分层BGM、低语、棱彩羁绊特效 |
| 存档与本地进度 | 存档读写、版本迁移、解锁状态 | JSON本地存档 | 存档校验、备份、版本兼容 |
...
建议文档：`run_combat_loop_design.md`
...
3. 7层棱彩：额外解锁、装填、互斥、冷却、数值上限。
4. 羁绊效果如何提交到 `ModifierStack`。
5. 羁绊UI：当前层数、下一档预览、棱彩锁定/装填显示。

优先级：高。MVP先做2套羁绊，暂不开放7层。

### 4.6 敌人与波次模块

建议文档：`enemy_wave_design.md`

核心内容：
1. 敌人基础属性、移动AI、碰撞、受击、死亡。
2. 怪物分类：普通怪、精英怪、Boss。
3. 波次生成器：时间驱动、刷怪区域、数量上限、强度曲线。
4. 性能策略：对象池、屏幕外处理、千怪同屏预算。
5. 敌人掉落与经验奖励。

优先级：高。MVP需要普通怪和一个阶段Boss。
...
建议文档：`camp_meta_progression_design.md`

核心内容：
1. 营地定位：固定2D界面，不做自由建造、生产链和资源采集。
2. 建筑设计：记忆方碑、武备工坊、遗物陈列架、观星台、治愈篝火。
3. 营地建筑升级：星点消耗、属性提升、特殊解锁。
4. 建筑升级：虚空残片消耗、等级上限、效果映射到局内参数。
5. 解锁系统：角色、武器、羁绊7层权限、图鉴、无尽纪录。

优先级：中高。MVP可先做营地入口和3个建筑。

### 4.11 UI交互模块

建议文档：`ui_flow_design.md`

核心内容：
1. 战斗HUD：血量、时间、负载、经验、羁绊面板。
2. 弹窗：升级三选一、负载替换、结算、暂停。
3. 局外界面：营地、营地升级树、角色选择、模式选择、图鉴。
4. 输入与暂停：鼠标/键盘/手柄适配优先级。
5. UI数据绑定：UI只读取状态，不直接修改核心规则。

优先级：高。升级弹窗和负载替换直接影响核心体验。

### 4.12 音频与氛围表现模块

建议文档：`audio_atmosphere_design.md`
...
建议文档：`save_progress_design.md`

核心内容：
1. 存档字段：星点、虚空残片、建筑等级与升级项、解锁、纪录。
2. 存档时机：结算、营地升级、营地升级购买、解锁发生时。
3. 存档版本：版本号、缺省字段补全、迁移策略。
4. 数据安全：临时文件写入、失败回滚、损坏存档处理。
5. 与 `DataRegistry` 的关系：存档只保存进度ID，不复制配置表完整内容。
...
4. 敌人与波次：1种普通怪 + 1种精英/BOSS。
5. 遗物与羁绊：2套羁绊，只开放2/3/5层。
6. 区域驻守与福缘收割：3个固定区域、基础连驻惩罚、换区收割。
7. 掉落与奖励：经验球、升级三选一、结算星点、福缘收割奖励。
8. 局外营地：固定营地界面 + 记忆方碑 + 武备工坊 + 治愈篝火。
9. UI：战斗HUD、升级弹窗、负载替换弹窗、休整选区弹窗、简易结算面板。
