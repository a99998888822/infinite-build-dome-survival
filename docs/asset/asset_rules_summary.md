...
| 文件 | 当前检查结果 | 建议 |
|---|---|---|
| `assets/sprites/player/void_hunter_idle_right.png` | 已存在，尺寸 256x256，透明背景正常，风格可用 | 保留原始素材；战斗使用 `combat/` 下的 54x54 派生版本 |
| `assets/sprites/player/void_hunter_walk_right_spritesheet.png` | 已存在，尺寸 1024x256，透明背景，当前为 4 帧版本 | 保留原始素材；战斗使用 `combat/` 下的 216x54 四帧派生版本 |
...
7. 同一套动画的源图帧尺寸必须一致。当前战斗使用按显示尺寸统一整理的派生贴图，以 1 倍最近邻采样显示；生成方式见 [战斗精灵清晰度调整](combat_sprite_clarity.md)。
...
2026-09-23 补充：裂甲奔袭者为原创大体型精英，约 10% 克苏鲁元素。使用 128×128 原生战斗帧、23 色硬透明像素图，另提供最近邻 2 倍展示版；六组动作共 23 帧。该精英遵循 [独立制作规格](elite_rusher_assets.md)，不受下表普通敌人“小体型、可仅单帧”的 MVP 约定限制。冲刺路径单独制作，为红色 30% 不透明度的直角矩形，无箭头、无圆角，不烘焙进角色帧表。
