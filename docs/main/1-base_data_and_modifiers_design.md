...
| 掉落与成长 | `finance` | 理财本金；每波开始前可存入的局内金币，第一波不可存入 | 玩家/全局 |
| 掉落与成长 | `interest_rate` | 利率；默认 5，波次结束按理财本金结算收益 | 玩家/全局 |
...
1. 配置表、设计文档、存档中的所有数值统一使用整数，不使用 `0.1`、`1.5` 这类小数表达比例。
...
1. 从第 2 波开始，每波攻击开始前，玩家可以将局内金币存入理财。
2. 存入理财的金币在本波战斗期间不可使用。
3. 本波攻击结束后，返还理财本金，并额外获得 `ceil(finance * interest_rate / 100)` 局内金币。
4. `interest_rate` 默认值为 `5`，表示默认利率 5%。
5. 理财收益属于局内金币收益；是否再吃 `currency_gain_percent` 由后续局内经济模块统一确认和实现，当前基础模块只提供属性和基础收益公式。
