...
| 范围与控制 | `area_size` | 攻击距离加成；决定武器能够攻击到多远的怪物 | 武器/玩家 |
| 范围与控制 | `damage_area_size` | 伤害范围加成；影响指定范围伤害的影响半径与对应视觉大小 | 武器/玩家 |
...
2. 百分比字段直接写百分数整数：`damage_percent = 10` 表示伤害 +10%；`area_size = 40` 表示攻击距离 +40%；`damage_area_size = 10` 表示范围伤害 +10%。
...
1. `area_size` 只表示攻击距离/索敌距离，不表示范围伤害区域大小。
2. 最终攻击距离使用：`最终攻击距离 = 武器基础攻击距离 * (1 + area_size / 100)`。
3. `damage_area_size` 只表示范围伤害区域大小，不改变武器寻找目标的距离。
4. 最终范围伤害半径使用：`最终伤害范围 = 基础伤害范围 * (1 + damage_area_size / 100)`。
5. `damage_area_size` 生效于电浆球、落雷、火焰、冰冻、爆裂；木质弓箭与电火花连锁不生效。
6. `pickup_radius` 只控制经验球和掉落物吸附范围，不参与武器攻击距离或伤害范围计算。

### 6.2.3 理财与利率

1. 第一波开始前不打开理财页；第 2 波起，每波开始前玩家可以打开理财页，选择存入、取出或跳过。
