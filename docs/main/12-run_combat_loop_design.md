...
3. `wave_end_absorb`、`interest_settlement`、`shop_popup`、`finance_popup`、`zone_select`、`zone_harvest_result` 必须按固定顺序执行，其中 `finance_popup` 在下一波开始前打开。
4. 同一时刻只允许一个模态弹窗存在。
...
- 能从启动页正确进入战斗或营地。
- 战斗中升级会立刻弹出共享奖励/商店页。
- 波次结束后会先吸收经验球并清理其余奖励拾取物，再结算利息，再开商店；下一波开始前再开理财。
- 战斗中不会正式存档。
- 死亡或通关后会完成最终结算并回到主入口。
