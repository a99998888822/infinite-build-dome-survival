...
2. 严格固定局内弹窗顺序，避免升级、商店、理财、结算互相打架。
...
| `level_up_popup` | 升级弹窗 | 暂停战斗并显示升级选项 | 玩家完成选择 |
| `wave_end_absorb` | 波次收尾吸收 | 吸收全部经验球，统一结算经验 | 吸收完成 |
| `interest_settlement` | 理财结算 | 返还本金并结算利息 | 结算完成 |
| `shop_popup` | 商店页面 | 展示当前波次商店候选 | 玩家购买或跳过 |
| `finance_popup` | 理财页面 | 允许玩家存入剩余局内金币 | 玩家确认 |
...
2. `level_up_popup` 优先级高于普通战斗。
3. `wave_end_absorb`、`interest_settlement`、`shop_popup`、`finance_popup` 必须按固定顺序执行。
4. 同一时刻只允许一个模态弹窗存在。
...
- 第 1 波开始前不可存入理财。
- 从第 2 波开始，玩家可以把剩余局内金币存入理财。
- 本波结束时返还本金，并额外获得 `ceil(finance * interest_rate / 100)` 的局内金币。
