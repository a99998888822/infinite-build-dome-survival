...
- [x] 主流程状态机已包含启动页、角色选择、战斗准备、波次战斗、升级弹窗、波次收尾、利息结算、商店、理财、战斗结果、营地入口。
- [x] 支持绑定 `PlayerController`、`WeaponLoadout`、`WaveManager`。
- [x] 支持角色确认后初始化初始武器。
- [x] 支持战斗中升级弹窗与波次结束后的升级补弹窗。
- [x] 支持波次结束后顺序推进到利息、商店、理财和下一波准备。
- [x] 支持死亡后进入简单战斗结果页。
- [x] 支持营地流程入口切换。
