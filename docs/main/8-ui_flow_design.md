...
1. 统一战斗内与局外界面的组织方式。
2. 为 HUD、共享奖励/商店页、营地界面提供一致的状态入口。
...
4. 战斗暂停、继续、进入共享奖励/商店页、进入营地的界面流转。
5. 局外营地界面与建筑入口。
...
1. 波次倒计时结束。
2. 统一吸取场上所有经验球。
3. 如果吸取经验后触发升级，则立即弹出升级页面；多次升级按顺序处理。
4. 结算上一波理财利息。
5. 弹出局内商店页面。
6. 玩家完成购买或跳过后，弹出理财页面。
7. 玩家自行决定是否把当前剩余局内金币存入理财。
8. 关闭理财页面后进入下一波战斗。
9. 若本局死亡或通关，则进入最终结算；只有最终结算才允许写入局外存档。
...
7. 免费奖励和商店购买共用第九模块的奖励选项 prefab；按钮文本由入口决定，免费升级奖励显示“选择”，商店购买显示具体花费金额。
...
商店只提供三类选项：`new_weapon` 新武器、`relic` 遗物、`weapon_upgrade` 已装备武器的下一等级升级项；不提供单独购买基础属性。免费奖励页、免费商店和付费商店共用同一套候选池、稀有度权重、类型权重和去重规则，区别只在于选择后是否扣除局内货币。
